...
import { ActivityDocument, LapData } from '@/lib/models/activity';

interface ActivityLapEditorProps {
    activity: ActivityDocument;
    isOpen: boolean;
    onClose: () => void;
    onUpdate: (_activity: ActivityDocument) => void;
}
...
export default function ActivityLapEditor({ activity, isOpen, onClose, onUpdate }: ActivityLapEditorProps) {
    ...
    const { control, handleSubmit, formState: { errors }, register } = useForm<FormValues>({
        ...
        defaultValues: {
            laps: (activity.laps || []).map((lap: LapData) => ({
    ...
    const onSubmit = async (data: FormValues) => {
        ...
        try {
            ...
            const updatedLaps = data.laps.map((item, newIndex: number) => {
                // Find original lap by the preserved lapIndex
                const originalLap = (activity.laps || []).find((l: LapData) => l.lapIndex === item.lapIndex);

                // Base object (original data or fallback if somehow missing)
                const base: Partial<LapData> = originalLap || {};
                                    ...
                                    <div className="space-y-1">
                                        <input
                                            ...
                                            className={cn(
                                                ...
                                                (errors.laps as any)?.[index]?.distanceKm ? "border-red-500 focus:ring-red-500" : "border-slate-700"
                                            )}
                                            placeholder={isSwim ? "0" : "0.00"}
                                        />
                                        {(errors.laps as any)?.[index]?.distanceKm && (
                                            <p className="text-[10px] text-red-400 pl-1">{(errors.laps as any)[index].distanceKm.message}</p>
                                        )}
                                    </div>
                                    <div className="space-y-1">
                                        <Controller
                                            ...
                                            render={({ field: { value, onChange } }) => (
                                                <input
                                                    ...
                                                    className={cn(
                                                        "w-full bg-slate-800 border rounded-lg px-3 py-2 text-white font-mono text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-all",
                                                        (errors.laps as any)?.[index]?.timeStr ? "border-red-500 focus:ring-red-500" : "border-slate-700"
                                                    )}
                                                    placeholder="MM:SS"
                                                />
                                            )}
                                        />
                                        {(errors.laps as any)?.[index]?.timeStr && (
                                            <p className="text-[10px] text-red-400 pl-1">{(errors.laps as any)[index].timeStr.message}</p>
                                        )}
                                    </div>
                                </div>
