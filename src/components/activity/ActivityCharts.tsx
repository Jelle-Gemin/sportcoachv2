...
import { StreamData } from '@/lib/models/activity';
...
const ActivityCharts = ({ streams, type }: ActivityChartsProps) => {
    ...
    const data = (streams.time || []).map((t: number, i: number) => ({
        time: t,
    ...
    })).filter((_: any, i: number) => i % 5 === 0); // Downsample for performance (every 5th point)
