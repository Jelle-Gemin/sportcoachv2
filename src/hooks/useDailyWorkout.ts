 ...
 */
export const useDailyWorkout = (date: Date) => {
 ...
 */
function enrichWorkout(basicWorkout: AnyWorkout) {
    const isSwim = basicWorkout.type === 'Swim';

    // Helper to format distance based on sport
    const formatDistString = (distInMeters: number) => {
    ...
    if (basicWorkout.workout) {
        const w = basicWorkout.workout as any; // Cast to any to handle various property sets
        const intervals = [];
        let step = 1;
        ...
        if (w.main && w.main.exercises) {
            (w.main.exercises as string[]).forEach((ex: string, idx: number) => {
                intervals.push({
                    step: step++,
                    name: ex,
    ...
    const template = workoutTemplates[basicWorkout.type as keyof typeof workoutTemplates] || workoutTemplates.Rest;
 ...
 */
function generateIntensityProfile(workout: any) {
    const profile = [];
