...
const BiometricInput = ({ label, field: { value, onChange }, editing, placeholder, error }) => (
...
const ZoneTable = ({ title, zones, type, editing, onChange, isOverridden, onReset }) => {
...
const GoalCard = ({ goal, editing, control, index, onDelete, errors }) => {
...
export default function Profile() {
    ...
    const {
        ...
        defaultValues: {
            biometrics: { weight: '', height: '', restingHR: '', maxHR: '', ftp: '', css: '' },
            trainingZones: { hr: [], cycling: [], swimming: [] },
            seasonGoals: []
        },
        mode: 'onChange'
    });
    ...
    const handleBiometricChange = useCallback((field, value) => {
        setValue(`biometrics.${field}`, value, { shouldValidate: true });

        // Auto-calculate zones if not overridden
        const currentBio = { ...biometrics, [field]: value };

        if ((field === 'maxHR' || field === 'restingHR') && !overrides.hr) {
            const newZones = calculateKarvonenZones(currentBio.maxHR, currentBio.restingHR);
            if (newZones.length > 0) setValue('trainingZones.hr', newZones);
        }
        if (field === 'ftp' && !overrides.cycling) {
            const newZones = calculateCogganZones(currentBio.ftp);
            if (newZones.length > 0) setValue('trainingZones.cycling', newZones);
        }
        if (field === 'css' && !overrides.swimming) {
            const newZones = calculateSwimZones(currentBio.css);
            if (newZones.length > 0) setValue('trainingZones.swimming', newZones);
        }
    }, [biometrics, overrides, setValue]);

    const handleZoneChange = (type, index, field, value) => {
        const newZones = [...trainingZones[type]];
        newZones[index] = { ...newZones[index], [field]: value };
        setValue(`trainingZones.${type}`, newZones);
        if (!overrides[type]) setOverrides(prev => ({ ...prev, [type]: true }));
    };

    const resetZones = (type) => {
        const bio = biometrics;
        let calculated = [];
        if (type === 'hr') calculated = calculateKarvonenZones(bio.maxHR, bio.restingHR);
        if (type === 'cycling') calculated = calculateCogganZones(bio.ftp);
        if (type === 'swimming') calculated = calculateSwimZones(bio.css);

        setValue(`trainingZones.${type}`, calculated);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <section className="space-y-4 md:col-span-2">
                    ...
                    <Card className="p-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
                        <Controller name="biometrics.weight" control={control} render={({ field }) => <BiometricInput label="Weight (kg)" field={field} editing={isEditing} error={errors.biometrics?.weight} />} />
                        <Controller name="biometrics.height" control={control} render={({ field }) => <BiometricInput label="Height (cm)" field={field} editing={isEditing} error={errors.biometrics?.height} />} />
                        <Controller
                            ...
                            render={({ field }) => (
                                <BiometricInput
                                    ...
                                    field={{
                                        ...field,
                                        onChange: (e) => handleBiometricChange('restingHR', e.target.value)
                                    }}
                                    editing={isEditing}
                        ...
                        <Controller
                            ...
                            render={({ field }) => (
                                <BiometricInput
                                    ...
                                    field={{
                                        ...field,
                                        onChange: (e) => handleBiometricChange('maxHR', e.target.value)
                                    }}
                                    editing={isEditing}
                        ...
                        <Controller
                            ...
                            render={({ field }) => (
                                <BiometricInput
                                    ...
                                    field={{
                                        ...field,
                                        onChange: (e) => handleBiometricChange('ftp', e.target.value)
                                    }}
                                    editing={isEditing}
                        ...
                        <Controller
                            ...
                            render={({ field }) => (
                                <BiometricInput
                                    ...
                                    field={{
                                        ...field,
                                        onChange: (e) => handleBiometricChange('css', e.target.value)
                                    }}
                                    editing={isEditing}
