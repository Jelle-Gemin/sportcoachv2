...
const parseDateSafe = (dateStr: string) => {
    ...
    return new Date(parseInt(parts[0]), parseInt(parts[1]) - 1, parseInt(parts[2]), 12, 0, 0); // Noon to avoid timezone issues
};
...
// ------------------------------------------------------------------
function SortableWorkoutItem({ item, isOverlay }: { item: any, isOverlay: boolean }) {
    const {
        attributes,
        listeners,
        setNodeRef,
...
function MonthDayCell({ day, isToday, isCurrentMonth }: MonthDayCellProps) {
    ...
    return (
        <div className={cn("min-h-[100px] border border-slate-800 p-2 flex flex-col gap-1 transition-colors hover:bg-slate-900/50",
            ...
            <div className="flex flex-col gap-1 mt-1 overflow-y-auto max-h-[80px] no-scrollbar">
                {hasWorkouts && day.workouts.map((w: any, i: number) => (
...
function DayColumn({ dayDate, items, isToday, onDayClick }: DayColumnProps) {
    ...
    return (
        <div ref={setNodeRef} className={cn("flex flex-col gap-2 min-h-[150px] rounded-xl p-2 md:p-3 transition-colors",
            ...
            <SortableContext
                id={dayDate}
                items={items.map((i: any) => i._id)} // Pass IDs only
                strategy={verticalListSortingStrategy}
            >
                <div className="flex flex-col gap-3 min-h-[50px]">
                    {items.map((item: any) => (
                        <SortableWorkoutItem key={item._id} item={item} isOverlay={false} />
                    ))}
                </div>
            </SortableContext>
        </div>
...
function PlanContent() {
    ...
    // Canonical URL Redirect: Ensure 'view' param is always present
    useEffect(() => {
    ...
    // Stabilize Date object for hook dependencies
    const memoizedDate = useMemo(() => parseDateSafe(selectedDate), [selectedDate]);

    // Data Hooks
    const { workouts: plannedWorkouts } = useWeeklyWorkouts(memoizedDate);
    const { calendarGrid } = useMonthlyWorkouts(memoizedDate);
    ...
    const workouts = useMemo(() => {
        ...
        const merged = list.map((planned: any) => {
            const actual = activities.find((act: any) => {
    ...
    const handleDragEnd = (event: any) => {
        ...
        if (newDateStr && newDateStr !== activeWorkout.fullDate) {
            setLocalWorkouts((prev: any[]) => {
                const updated = prev.map((w: any) => {
                    if (w._id === active.id) {
                        const d = parseDateSafe(newDateStr as string);
                        return {
                            ...w,
                            fullDate: newDateStr,
    ...
    const weekNumber = useMemo(() => {
        ...
        const diffTime = Math.abs(current.getTime() - startOfPlan.getTime());
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    ...
                    <div className="flex bg-slate-900 rounded-xl p-1 border border-slate-800 flex-1 md:flex-none">
                        <Link
                            ...
                            replace
                            className={cn(
                                "flex-1 md:flex-none px-4 py-2 text-sm font-bold rounded-lg transition-all text-center",
                                viewMode === 'week' ? "bg-[#3b82f6] text-white shadow-lg shadow-blue-500/20" : "text-slate-500 hover:text-slate-300"
                            )}
                        >
                            Week
                        </Link>
                        <Link
                            ...
                            className={cn(
                                "flex-1 md:flex-none px-4 py-2 text-sm font-bold rounded-lg transition-all text-center",
                                viewMode === 'month' ? "bg-[#3b82f6] text-white shadow-lg shadow-blue-500/20" : "text-slate-500 hover:text-slate-300"
                            )}
                        >
                            Month
                        </Link>
                ...
                <Card className="p-0 overflow-hidden bg-slate-900 border-slate-800 touch-pan-y">
                    <div
                        ...
                        onTouchEnd={(e: React.TouchEvent) => {
                            const isDragging = e.currentTarget.getAttribute('data-is-dragging');
                            if (isDragging !== "true") return;
                            e.currentTarget.setAttribute('data-is-dragging', "false");

                            const startXStr = e.currentTarget.getAttribute('data-start-x');
                            if (!startXStr) return;
