...
export async function POST(request) {
    try {
        ...
        const stravaActivityIds = stravaActivities.map(a => a.id);
        const existingIds = await findExistingStravaIds(stravaActivityIds);

        // Filter out what we already have
        const missingActivities = stravaActivities.filter(a => !existingIds.includes(a.id));
        ...
        await Promise.all(batchToProcess.map(async (activitySummary) => {
            try {
                ...
                errors.push({ id: activitySummary.id, error: err.message });
        ...
        return NextResponse.json(
            { error: 'Failed to sync activities', details: err.message },
            { status: 500 }
        );
    }
}
...
function transformToSchema(detail, laps, streams, stravaId) {
    return {
        stravaId: detail.id,
